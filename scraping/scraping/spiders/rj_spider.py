    ...
    def start_requests(self):
        for i in range(132):
            yield scrapy.Request(
                url=f'https://loft.com.br/venda/imoveis/rj/rio-de-janeiro?pagina={i}', 
                meta=dict(
                    playwright=True,
    ...
    async def parse(self, response):
        ...
        for property in response.css('div.jss277'):
            yield {
                ...
                'address': property.css('div.jss277 h2::text').get().split(', ')[0].replace('R.', 'Rua').replace('Av.', 'Avenida').replace(' ', '_').rstrip('_0123456789'),
                'neighborhood': property.css('div.jss277 h2::text').get().split(', ')[1].replace(' ', '_'),
